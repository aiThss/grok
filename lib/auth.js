...
export function authConfigurationError() {
  if (!passwordIsConfigured()) return "APP_PASSWORD has not been configured on the server.";
  const secret = process.env.SESSION_SECRET;
  if (!secret || secret.length < 32) return "SESSION_SECRET must be at least 32 characters long.";
  return null;
}
