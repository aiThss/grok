...
export async function openChatStream({ messages, model }) {
  const response = await grokFetch("/chat/completions", {
    method: "POST",
    body: JSON.stringify({
      model: lockedModel(model),
      // Streaming sends headers and the first token as soon as the upstream
      // starts responding. That prevents a reverse proxy from timing out
      // while it waits for one large, buffered JSON response.
      stream: true,
      messages: [
        {
          role: "system",
  ...
  if (!response.body) throw new Error("The Grok gateway returned an empty streaming response.");

  const contentType = response.headers.get("content-type") || "";
  if (!contentType.includes("text/event-stream")) {
    const detail = await response.text();
    throw gatewayError(`Grok gateway did not return a chat stream: ${formatGatewayDetail(detail)}`);
  }
